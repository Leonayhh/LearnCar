...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    CGFloat textLayerInterval = self.frame.size.width / ( showCount * 2);
    
    CGFloat separatorLineInterval = self.frame.size.width / showCount;
    
    CGFloat bgLayerInterval = self.frame.size.width / showCount;
    
    self.contentSize=CGSizeMake(_numOfMenu*self.frame.size.width/showCount, 0);
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0, self.frame.size.width/self.showCount, self.frame.size.height-1);
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    CGFloat sizeWidth = (size.width < (self.frame.size.width / showCount) - 25) ? size.width : self.frame.size.width / showCount - 25;
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    //calculate index
    NSInteger tapIndex = touchPoint.x / (self.frame.size.width / showCount);
