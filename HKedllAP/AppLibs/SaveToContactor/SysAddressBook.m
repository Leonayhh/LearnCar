...
#import "SysAddressBook.h"

@implementation SysAddressBook



// 单列模式
+ (SysAddressBook*)shareControl
{
    static SysAddressBook *share;
    @synchronized(self) {
        if(!share ) {
            share = [[SysAddressBook alloc] init];
        }
    }
    return share;
}
...
{
    return [[SysAddressBook shareControl] addContactName:name phoneNum:num withLabel:label];
}
...
{
    return [[SysAddressBook shareControl] existPhone:phoneNum];
}
