...
@property (nonatomic, weak, readwrite) FaceView *emotionManagerView;
...
- (FaceView *)emotionManagerView {
    if (!_emotionManagerView) {
        FaceView *emotionManagerView = [[FaceView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
        emotionManagerView.delegate = _messageInputView.inputTextView;
        emotionManagerView.backgroundColor = [UIColor colorWithRed:245.0/255.0 green:244.0/255.0 blue:247.0/255.0 alpha:1];
    ...
    void (^PickerMediaBlock)(UIImage *image, NSDictionary *editingInfo) = ^(UIImage *image, NSDictionary *editingInfo) {
        if (image) {
            ...
            } else
            {
                ...
                UIImage *_editImage=nil;
                UIImage *_originalImage = [editingInfo valueForKey:UIImagePickerControllerOriginalImage];

                //不能原图 内存吃不消
                if (_originalImage.size.width>=320*2)
                {
                    
                    
                    _editImage=[self scaleToSize:_originalImage size:CGSizeMake(320*2, _originalImage.size.height*320*2/_originalImage.size.width)];
                    
                }
                else
                {
                    
                    _editImage=[self scaleToSize:_originalImage size:CGSizeMake(_originalImage.size.width, _originalImage.size.height)];
                    
                }

                
                [weakSelf didSendMessageWithPhoto:_editImage];
