...
- (void)playAudioWithFileName:(NSString*)fileName {
    ...
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            wavName =[VoiceConverter amrToWav:fileName];
            ...
            if (wavName!=nil)
            {
                ...
                AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:wavName] error:nil];
                pl.delegate = self;
                [pl play];
                self.player = pl;
                [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag {
    ...
    if (self.playingFileName!=nil && ![self.playingFileName isEqualToString:@""])
    {
        
        //yes
        if ([XHVoiceCommonHelper deleteFileAtPath:self.playingFileName])
        {
